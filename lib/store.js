...
// Maximum visitor events retained in the detailed log (newest first).
// High enough to keep the entire history of a portfolio site.
...
export async function trackView({ path, country, city, region, device, browser, os }) {
  ...
  // Keep the full visitor history (newest first) so early tracking is never
  // lost. Capped very high only to bound the blob size — effectively "all"
